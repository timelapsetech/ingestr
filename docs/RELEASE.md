...
| Location | Keys |
|----------|------|
| `Ingestr.xcodeproj` → Ingestr target | `MARKETING_VERSION` (user-facing, e.g. `1.5`), `CURRENT_PROJECT_VERSION` (build number, e.g. `7`) |
...
Upload `Ingestr.zip` to a [GitHub Release](https://github.com/timelapsetech/ingestr/releases) tagged `v1.5` (match `MARKETING_VERSION`). The site and README link to:
...
```bash
git tag -a v1.5 -m "Ingestr 1.5"
git push origin v1.5
```

## 6. After release

- Confirm **Open Folder**, **Auto Rename + Auto Split**, and output-folder permissions on a real card dump.
